...
public class MealDaoSQL {
    ...
    public List<MealRequestModel> searchMeal(String name) {
        List<MealRequestModel> list = new ArrayList<>();

        try {
            Statement st = conn.createStatement();
            ResultSet rs = st.executeQuery("SELECT DISTINCT m.* FROM meal m INNER JOIN " +
                    "mealfood mf ON mf.meal_id = m.meal_id INNER JOIN food f on mf.food_id = f.food_id " +
                    "WHERE LOWER(meal_name) LIKE '%" + name + "%'");
